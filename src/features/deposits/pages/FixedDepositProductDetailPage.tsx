    ...
    <div className="p-6 max-w-5xl m-auto space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent className="divide-y divide-gray-100 dark:divide-gray-800">
            <InfoRow icon={<Repeat className="h-4 w-4" />} label={t("Compounding Period")} value={COMPOUNDING_LABELS[p.interestCompoundingPeriodType?.id] ?? enumVal(p.interestCompoundingPeriodType, "—")} />
            <InfoRow icon={<CalendarClock className="h-4 w-4" />} label={t("Posting Period")} value={POSTING_LABELS[p.interestPostingPeriodType?.id] ?? enumVal(p.interestPostingPeriodType, "—")} />
            <InfoRow icon={<CalendarClock className="h-4 w-4" />} label={t("Calculation Method")} value={CALCULATION_LABELS[p.interestCalculationType?.id] ?? enumVal(p.interestCalculationType, "—")} />
            <InfoRow icon={<CalendarClock className="h-4 w-4" />} label={t("Days in Year")} value={DAYS_IN_YEAR_LABELS[p.interestCalculationDaysInYearType?.id] ?? enumVal(p.interestCalculationDaysInYearType, "—")} />
