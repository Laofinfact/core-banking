...
import type {
  ...
  CollateralProductUpdateRequest,
  CollateralProductCommandResponse,
  CollateralProductTemplate,
} from "../types/collateralProduct";
...
export async function fetchCollateralProductTemplate(): Promise<CollateralProductTemplate> {
  const { data } = await client.get<CollateralProductTemplate>(
    "/collateral-management/template",
  );
  return data;
}

export async function createCollateralProduct(
  payload: CollateralProductCreateRequest,
): Promise<CollateralProductCommandResponse> {
  const { data } = await client.post<CollateralProductCommandResponse>(
...
export async function updateCollateralProduct(
  id: number | string,
  payload: CollateralProductUpdateRequest,
): Promise<CollateralProductCommandResponse> {
  const { data } = await client.put<CollateralProductCommandResponse>(
    `/collateral-management/${id}`,
    payload,
  );
  return data;
}
